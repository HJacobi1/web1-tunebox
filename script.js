  ...
  albuns.forEach((album) => {
    let albumCard = $(`
      <div class="album-card" data-album-id="${album.id}">
        <img src="${album.capa}" alt="${album.nome} Cover" class="album-cover" />
...
function updateStarRating(containerId, rating, isHover = false) {
  ...
  if (label.length === 0) {
    if (rating > 0) {
      $(containerId).after(`<span class="rating-label">${rating} de 5 estrelas</span>`);
    }
  } else {
    if (rating > 0) {
...
function displayReview(review) {
  ...
  if (rating > 0) {
    ...
    for (let i = 1; i <= 5; i++) {
      starsHtml += `<span class="star-rating ${i <= rating ? 'selected' : ''}">★</span>`;
    }
  ...
  if (review.data || review.createdAt) {
    ...
    try {
      ...
      if (!isNaN(date.getTime())) {
        dateHtml = `<p class="review-date">${date.toLocaleDateString('pt-BR')}</p>`;
      }
    } catch (e) {
...
$(document).ready(async function () {
  ...
  function updateQuickReviewStars(rating = quickReviewStars, isHover = false) {
    ...
    if (label.length === 0) {
      if (rating > 0 && !isHover) {
        $("#starPicker").after(`<span class="rating-label">${rating} de 5 estrelas</span>`);
      }
    } else {
      if (rating > 0 && !isHover) {
        label.text(`${rating} de 5 estrelas`);
  ...
  $("#reviewForm").on('submit', async function(e) {
    ...
    if (!reviewText && quickReviewStars === 0) {
      alert("Por favor, escreva uma observação ou selecione uma avaliação com estrelas.");
      return;
    }
    ...
    try {
      if (existingReviewIndex !== -1) {
        ...
        await $.ajax(settings).done(function(response) {
          ...
          alert("Review atualizada com sucesso! (simulado)");
          resetQuickReviewForm();
        });
      } else {
        // Cria nova review
        ...
        await $.ajax(settings).done(function(response) {
          ...
          alert("Review salva com sucesso! (simulado)");
          resetQuickReviewForm();
        });
      }
    } catch (error) {
  ...
  function updateModalIfOpen(albumId, review) {
    ...
    if (String(currentAlbumId) === String(albumId) && $("#albumModal").is(":visible")) {
      // Atualiza a exibição da review na modal
      currentReviewId = review.id;
      selectedStars = review.estrelas || review.rating || 0;
      displayReview(review);
  ...
  $("#capa").on('change', function(e) {
    ...
    if (file) {
      ...
      reader.onload = function(e) {
        $("#capaPreview").html(`<img src="${e.target.result}" style="max-width: 200px; max-height: 200px; border-radius: 8px; margin-top: 8px;" />`);
      };
  ...
  async function saveAlbum(nome, data, artista, genero, capaUrl, musicas) {
    ...
    try {
      ...
      await $.ajax(settings).done(function(response) {
        ...
        albuns.push(newAlbum);
        updateAlbumsDisplay();
        alert("Álbum salvo com sucesso! (armazenado localmente)");
        resetAlbumForm();
      });
    } catch (error) {
        ...
        await $.ajax(settings).done(function(response) {
          ...
          if (index !== -1) {
            ...
            const updatedReview = {
              ...
              rating: response.rating || reviewData.rating || response.estrelas || reviewData.estrelas || 0,
              texto: response.texto || reviewData.texto || response.text || reviewData.text || ""
            };
            reviews[index] = updatedReview;
            selectedStars = updatedReview.estrelas || updatedReview.rating || 0;
